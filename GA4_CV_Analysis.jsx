...
const DateRange = ({ startDate, endDate, minDate, maxDate, onChange }) => (
...
const DrilldownTable = ({ data }) => {
  ...
  // Chart title based on current drilldown level
...
/* ============================================================
   MAIN APP
============================================================ */
export default function GA4ConversionApp() {
  ...
  const filteredData = useMemo(() => {
    if (!rawData) return [];
    return rawData.filter((r) => r.date >= startDate && r.date <= endDate);
  ...
  const summary = useMemo(() => {
    ...
    const t = agg(filteredData);
    const channels = _.uniq(filteredData.map((r) => r.channel));
    return { ...t, channels: channels.length, rows: filteredData.length };
  }, [filteredData]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 flex items-center justify-center p-8">
        <div className="bg-white rounded-2xl shadow-lg p-10 text-center max-w-md">
          ...
          <h1 className="text-xl font-bold text-gray-800 mb-2">GA4 コンバージョン分析</h1>
          <p className="text-sm text-gray-500 mb-6">
            GA4の広告イベントCSVをアップロードして、<br />
            媒体別・キャンペーン別のCV獲得状況を分析します
          </p>
          <label className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-lg cursor-pointer hover:bg-indigo-700 transition-colors text-sm font-medium shadow-sm">
            <span>📁 CSVを選択</span>
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4">
            <h1 className="text-base font-bold text-gray-800">📊 GA4 CV分析</h1>
            <DateRange startDate={startDate} endDate={endDate} minDate={dateRange.min} maxDate={dateRange.max}
              onChange={(s, e) => { setStartDate(s); setEndDate(e); }} />
          </div>
          <label className="inline-flex items-center gap-1 px-3 py-1 bg-gray-100 text-gray-600 rounded text-xs cursor-pointer hover:bg-gray-200">
            <span>📁 CSV変更</span>
            <input type="file" accept=".csv" onChange={handleFile} className="hidden" />
          </label>
        </div>
