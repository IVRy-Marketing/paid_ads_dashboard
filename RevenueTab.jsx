...
// ============================================================
// カレンダーコンポーネント（広告ダッシュボードと同じUI）
// ============================================================
function CalendarPicker({ calMonth, setCalMonth, pickFrom, pickTo, onPick, onApply, onClose, allDates }) {
  const { y, m } = calMonth;
  ...
  const toStr = (day) => `${y}-${pad2(m + 1)}-${pad2(day)}`;
  ...
  return (
    <div className="absolute top-full right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-2xl z-30 p-3 w-64">
          ...
          return (
            <button key={day} onClick={() => hd && onPick(toStr(day))} disabled={!hd}
              className={`w-8 h-8 rounded-md text-xs font-medium mx-auto flex items-center justify-center transition
                ${fr || tr ? "bg-indigo-600 text-white"
                  : ir ? "bg-indigo-100 text-indigo-700"
                  : hd ? "hover:bg-gray-100 text-gray-700"
                  : "text-gray-300 cursor-default"}
                ${!fr && !tr && !ir && dow === 0 && hd ? "text-red-500" : ""}
                ${!fr && !tr && !ir && dow === 6 && hd ? "text-blue-500" : ""}`}>
              {day}
            </button>
          );
        })}
      ...
      <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between">
        <div className="text-xs text-gray-500">
          {pickFrom
            ? <span>{pickFrom}{pickTo ? ` 〜 ${pickTo}` : " 〜 ?"}</span>
            : <span className="text-gray-400">開始日をクリック</span>}
        </div>
        <div className="flex gap-1">
          <button onClick={onClose} className="px-2 py-1 text-xs text-gray-500 hover:bg-gray-100 rounded-md">閉じる</button>
          <button onClick={onApply} disabled={!pickFrom}
            className={`px-3 py-1 text-xs rounded-md font-medium ${pickFrom ? "bg-indigo-600 text-white hover:bg-indigo-700" : "bg-gray-200 text-gray-400"}`}>
            適用
          </button>
        </div>
      </div>
...
export default function RevenueTab() {
  ...
  const [calPickFrom, setCalPickFrom] = useState(null);
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setDrillFilters({});
      setSelectedBar(null);
      // カレンダーの初期月を最新データ月に合わせる
      const dates = parsed.filter(isPaidRow).map((r) => r.inquiry_date).filter(Boolean).sort();
      if (dates.length) {
        const last = new Date(dates[dates.length - 1]);
        setCalMonth({ y: last.getFullYear(), m: last.getMonth() });
      }
    };
  ...
  // ---------- selectedBar期間絞り込み ----------
  // バーをクリック中はその期間行だけをベースにしてドリルダウンする
  ...
  const handleDrillDown = (name) => {
    ...
    // selectedBarは維持したまま → 選択した期間内でドリルダウンし続ける
  };
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5 ml-auto">
          {[["yesterday","前日"],["thisMonth","当月"],["lastMonth","先月"],["30d","30日"],["60d","60日"],["all","全期間"]].map(([k, l]) => (
            <button key={k} onClick={() => { setPeriodMode(k); setCalendarOpen(false); setSelectedBar(null); }}
              className={`px-2 py-1 rounded-md text-xs font-medium transition ${periodMode === k ? "bg-white shadow text-gray-800" : "text-gray-500 hover:text-gray-700"}`}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div className="relative">
          <button
            onClick={() => {
              ...
              if (!calendarOpen) {
                setCalPickFrom(customFrom || null);
                setCalPickTo(customTo || null);
              }
            }}
          ...
          {calendarOpen && (
            <CalendarPicker
              ...
              onPick={(d) => {
                if (!calPickFrom || calPickTo || d < calPickFrom) {
                  setCalPickFrom(d); setCalPickTo(null);
                } else {
                  setCalPickTo(d);
                }
              }}
              onApply={() => {
                setCustomFrom(calPickFrom || "");
                setCustomTo(calPickTo || calPickFrom || "");
